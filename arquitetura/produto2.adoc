...
As provas de conceito apresentadas neste documento são de suma importância para o consumo e o desenvolvimento dos web services processados pelo SISLEGIS. Ou seja, tanto na obtenção de informações dos sites da Câmara e Senado (utilizando o framework `XStream`), quanto na geração de informações que serão utilizadas pela camada de apresentação do SISLEGIS (utilizando a API `JAX-RS`), as PoCs desenvolvidas deste produto serão utilizadas como referência para o desenvolvimento dessa aplicação.
